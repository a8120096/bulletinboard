...
<body>
    ...
    if (isset($_POST["name"]) && isset($_POST["comment"]) && isset($_POST["passward"])) {
        ...
        if ((!empty($_POST["name"])) && (!empty($_POST["comment"])) && (!empty($_POST["passward"])) && (!empty($_POST["id"]))) {
            ...
            $sql = 'UPDATE tbbbs SET name=:name,comment=:comment,passward=:passward,created_at=:created_at WHERE id=:id';
        ...
        if ((!empty($_POST["name"])) && (!empty($_POST["comment"])) && (!empty($_POST["passward"]) && (empty($_POST["id"])))) {
            // テーブルtbtestにレコードを追加
            $sql = $pdo->prepare("INSERT INTO tbbbs (name, comment, passward, created_at) VALUES (:name, :comment, :passward, :created_at)");
    ...
    if (isset($_POST["del_num"]) && isset($_POST["del_passward"])) {
        if ((!empty($_POST["del_num"])) && (!empty($_POST["del_passward"]))) {
            ...
            $sql = 'SELECT * FROM tbbbs WHERE id=:del_num';
            $stmt = $pdo->prepare($sql);                  // ←差し替えるパラメータを含めて記述したSQLを準備し、
            $stmt->bindParam(':del_num', $del_num, PDO::PARAM_INT); // ←その差し替えるパラメータの値を指定してから、
            $stmt->execute();
            ...
            if ($passward == $del_passward) {
                $sql = 'DELETE FROM tbbbs WHERE id=:del_num ';
                $stmt = $pdo->prepare($sql);                  // ←差し替えるパラメータを含めて記述したSQLを準備し、
                $stmt->bindParam(':del_num', $del_num, PDO::PARAM_INT); // ←その差し替えるパラメータの値を指定してから、
                $stmt->execute();
            }
    ...
    if (isset($_POST["edit_num"]) && isset($_POST["edit_passward"])) {
        if ((!empty($_POST["edit_num"])) && (!empty($_POST["edit_passward"]))) {
            ...
            $sql = 'SELECT * FROM tbbbs WHERE id=:edit_num';
            $stmt = $pdo->prepare($sql);                  // ←差し替えるパラメータを含めて記述したSQLを準備し、
            $stmt->bindParam(':edit_num', $edit_num, PDO::PARAM_INT); // ←その差し替えるパラメータの値を指定してから、
            $stmt->execute();
            $results = $stmt->fetchAll();
            foreach ($results as $row) {
                $passward =  $row['passward'];
